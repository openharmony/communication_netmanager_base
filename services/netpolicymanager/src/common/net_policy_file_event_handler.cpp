...
NetPolicyFileEventHandler::~NetPolicyFileEventHandler() = default;
...
{
    if (!GetEventRunner() || event == nullptr) {
    ...
    if (MSG_POLICY_FILE_WRITE == eventId) {
        ...
        commitWait_ = true;
        NETMGR_LOG_D("SendEvent MSG_POLICY_FILE_COMMIT[delay=%{public}d, now=%{public}s, this=%{public}p]", delay,
                     std::to_string(GetNowMilliSeconds()).c_str(), this);
...
bool NetPolicyFileEventHandler::DeleteBak()
{
    ...
    if (stat(POLICY_FILE_BAK_NAME, &buffer) == 0) {
        NETMGR_LOG_E("remove file on disk.");
