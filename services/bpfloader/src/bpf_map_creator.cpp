...
bool BpfMapCreator::CreateMaps(BpfMapData *maps, int32_t nrMaps)
{
    for (int32_t i = 0; i < nrMaps; i++) {
        ...
        maps[i].fd = mapFd_[i];
        NETNATIVE_LOGI("maps[%{public}d].fd: %{public}d, name:%{public}s", i, mapFd_[i], maps[i].name.c_str());
    ...
    for (const auto &section : elfio.sections) {
        ...
        for (ELFIO::Elf_Xword i = 0; i < symbols.get_symbols_num(); i++) {
            ...
            if (type == STT_OBJECT && name.find("_map") != std::string::npos) {
                ...
                maps[mapIndex].value = value;
                NETNATIVE_LOGI("maps[%{public}d].name: %{public}s, value: %{public}s",
                    mapIndex, maps[mapIndex].name.c_str(), std::to_string(maps[mapIndex].value).c_str());
                mapIndex++;
            }
        }
    }
...
int32_t BpfMapCreator::BpfCreateMapXattr(const BpfCreateMapAttr *createAttr) const
{
    ...
    if (!createAttr->name.empty()) {
        createAttr->name.copy(attr.map_name,
