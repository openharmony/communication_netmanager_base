...
class EthernetManagement : public NlkEventHandle {
    ...
    public:
        NativeNotifyCallback(EthernetManagement &ethernetManagement);
...
public:
    ...
    int32_t UpdateDevInterfaceLinkInfo(sptr<OHOS::NetdNative::DhcpResultParcel> &dhcpResult);
...
private:
    ...
    void SetDevState(sptr<DevInterfaceState> &devState, const std::string &devName, const std::vector<uint8_t> &hwAddr,
        bool up, bool lowerUp);
    sptr<OHOS::NetdNative::INetdService> GetNetdNativeProxy();

private:
    sptr<OHOS::NetdNative::INotifyCallback> nativeNotifyCallback_;
    sptr<OHOS::NetdNative::INetdService> netdNativeService_ = nullptr;
