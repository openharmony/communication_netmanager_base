 ...
 int32_t NetDnsResultCallback::OnDnsResultReport(uint32_t size, const std::list<NetsysNative::NetDnsResultReport> netDnsResultReport)
{
    ...
    //std::map<uint32_t, NetDnsResult> netDnsResult;
    ...
    for (auto &item : netDnsResult_) {
        NETMGR_LOG_I("netId_: %{public}d, totalReports_: %{public}d, failReports_: %{public}d", item.first, item.second.totalReports_, item.second.failReports_);
