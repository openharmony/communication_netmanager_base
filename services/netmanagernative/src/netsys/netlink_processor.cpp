...
void NetlinkProcessor::HandleAddressChange(const std::shared_ptr<NetlinkMessageDecoder> &message)
{
    ...
    if (!iface.empty() && iface[0] && !address.empty() && !flags.empty() && !scope.empty()) {
        if (addrUpdated) {
            OnInterfaceAddressUpdate(address, iface, ConvertToInt64(flags),
                                     ConvertToInt64(scope));
        } else {
            OnInterfaceAddressRemove(address, iface, ConvertToInt64(flags),
                                     ConvertToInt64(scope));
        }
    }
}
...
void NetlinkProcessor::HandleSubSysIdLetimer(const std::shared_ptr<NetlinkMessageDecoder> &message)
{
    ...
    if (!state.empty()) {
        ...
        int64_t processTimestamp =
            timestamp.empty() ? 0 : std::strtoll(timestamp.c_str(), nullptr, DECIMALISM);
        int32_t intLabel;
        if (ParseInt(label.c_str(), &intLabel)) {
            const int32_t reportedUid =
                (!uid.empty() && isActive) ? ConvertToInt64(uid) : -1;
...
void NetlinkProcessor::HandleRndssChange(const std::shared_ptr<NetlinkMessageDecoder> &message)
{
    ...
    if (!lifetime.empty() && !servers.empty()) {
        OnInterfaceDnsServersUpdate(iface, ConvertToInt64(lifetime),
                                    Split(servers, ","));
    }
}
...
void NetlinkProcessor::OnInterfaceAddressUpdate(const std::string &addr,
                                                ...
                                                int32_t scope)
{
    NETNATIVE_LOGI("OnInterfaceAddressUpdated: %{public}s, %{public}s, %{public}d, %{public}d", addr.c_str(),
...
void NetlinkProcessor::OnInterfaceAddressRemove(const std::string &addr,
                                                const std::string &ifName,
                                                int32_t flags,
                                                int32_t scope)
{
    NETNATIVE_LOGI("OnInterfaceAddressRemov: %{public}s, %{public}s, %{public}d, %{public}d", addr.c_str(),
                   ifName.c_str(), flags, scope);
    for (auto &callback : *netlinkCallbacks_) {
...
void NetlinkProcessor::OnInterfaceDnsServersUpdate(const std::string &ifName,
                                                   int64_t lifetime,
