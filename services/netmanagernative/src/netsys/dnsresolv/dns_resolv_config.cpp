...
void DnsResolvConfig::SetIpv6UidBlackList(const uint32_t &uid)
{
    ...
    if (it != ipv6UidBlackList_.end()) {
        ...
        NETNATIVE_LOGI("SetIpv6UidBlackList uid:%{public}u is exists, update time:%{public}lu", uid, now);
        return;
    }
    ...
    NETNATIVE_LOGI("SetIpv6UidBlackList uid:%{public}u, time:%{public}lu", uid, now);
}
