...
bool NetLinkSocketDiag::IsMatchNetwork(const inet_diag_msg *msg, const std::list<std::string> &ipAddrList)
{
    if (msg->idiag_family == AF_INET) {
    ...
    while (readBytes > 0) {
        ...
        for (nlmsghdr *nlh = reinterpret_cast<nlmsghdr *>(buf); NLMSG_OK(nlh, len); nlh = NLMSG_NEXT(nlh, len)) {
            if (nlh->nlmsg_type == NLMSG_ERROR) {
                ...
                SockDiagDumpCallback(proto, excludeLoopback, msg, ipAddrList);
            }
        }
...
int32_t NetLinkSocketDiag::DestroySocketsLackingNetwork(const std::list<std::string> &ipAddrList, bool excludeLoopback)
{
    ...
    for (const int family : {AF_INET, AF_INET6}) {
        ...
        ret = ProcessSockDiagDumpResponse(proto, excludeLoopback, ipAddrList);
        if (ret != NETMANAGER_SUCCESS) {
