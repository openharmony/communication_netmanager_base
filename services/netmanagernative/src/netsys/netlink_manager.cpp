...
void NetlinkManager::RegisterNetlinkCallback(sptr<NetsysNative::INotifyCallback> callback)
{
    if (callback == nullptr) {
...
std::unique_ptr<NetlinkProcessor> NetlinkManager::SetSocket(int32_t &sock,
                                                            int32_t netlinkType,
                                                            int32_t groups,
                                                            int32_t format,
                                                            bool configNflog)
...
{
    if ((ueventProc_ = SetSocket(ueventSocket_, NETLINK_KOBJECT_UEVENT, UEVENT_GROUP,
                                 NETLINK_FORMAT_ASCII, false)) == nullptr) {
    ...
    if ((quotaProc_ = SetSocket(quotaSocket_, NETLINK_NFLOG, NFLOG_QUOTA_GROUP, NETLINK_FORMAT_BINARY,
                                false)) == nullptr) {
        NETNATIVE_LOGE("Unable to open qlog quota socket, check if xt_quota2 can send via UeventHandler");
    }

    if ((strictProc_ = SetSocket(strictSocket_, NETLINK_NETFILTER, 0, NETLINK_FORMAT_BINARY_UNICAST,
                                 true)) == nullptr) {
