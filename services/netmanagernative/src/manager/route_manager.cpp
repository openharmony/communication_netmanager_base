...
std::map<std::string, uint32_t> RouteManager::interfaceToTable;

RouteManager::RouteManager()
{
    int status = ModifyRule(RTM_NEWRULE, ROUTE_LOCAL_NETWORK_TABLE, FR_ACT_TO_TBL, RULE_LOCAL_NETWORK_PRI);
    if (status < 0) {
        NETNATIVE_LOGE("RouteManager::RouteManager, add rule error");
    }
}

RouteManager::~RouteManager() {}

int RouteManager::ModifyRule(uint32_t type, uint32_t table, uint8_t action, uint32_t priority)
{
...
int RouteManager::AddInterfaceToDefaultNetwork(const char *interfaceName, NetworkPermission permission)
{
    NETNATIVE_LOGI("Entry RouteManager::AddInterfaceToDefaultNetwork, %{public}s", interfaceName);
...
int RouteManager::RemoveInterfaceFromDefaultNetwork(const char *interfaceName, NetworkPermission permission)
{
    NETNATIVE_LOGI("Entry RouteManager::AddInterfaceToDefaultNetwork, %{public}s", interfaceName);
...
int nmd::RouteManager::ReadAddrGw(const char *addr, InetAddr *res)
{
...
int nmd::RouteManager::ReadAddr(const char *addr, InetAddr *res)
{
...
int nmd::RouteManager::AddRoute(int netId, std::string interfaceName, std::string destination, std::string nextHop)
{
    NETNATIVE_LOGE("Entry nmd::RouteManager::AddRoute");
    nmd::NetlinkSocket netLinker;
    netLinker.Create(NETLINK_ROUTE);
    nmd::NetlinkMsg nlmsg(NLM_F_CREATE | NLM_F_EXCL, nmd::NETLINK_MAX_LEN, NetlinkManager::GetPid());
    struct rtmsg msg;
...
int RouteManager::RemoveRoute(int netId, std::string interfaceName, std::string destination, std::string nextHop)
{
    nmd::NetlinkSocket netLinker;
    netLinker.Create(NETLINK_ROUTE);
    nmd::NetlinkMsg nlmsg(NLM_F_CREATE | NLM_F_EXCL, nmd::NETLINK_MAX_LEN, NetlinkManager::GetPid());
...
uint32_t RouteManager::GetRouteTableForInterface(const char *interfaceName)
{
    ...
    if (table == 0) {
        NETNATIVE_LOGE(
            "[RouteManager] cannot find interface %{public}s, error:%{public}d", interfaceName, errno);
