...
{
    NETMGR_LOG_I("CompleteIsActivate");
    EthernetAsyncContext* context = static_cast<EthernetAsyncContext *>(data);
    if (context == nullptr) {
        NETMGR_LOG_E("context == nullptr");
        return;
...
{
    NETMGR_LOG_I("ExecGetActivateInterfaces");
    EthernetAsyncContext *context = (EthernetAsyncContext *)data;
    if (context == nullptr) {
        NETMGR_LOG_E("context == nullptr");
        return;
...
{
    NETMGR_LOG_I("CompleteGetActivateInterfaces");
    EthernetAsyncContext* context = static_cast<EthernetAsyncContext *>(data);
    if (context == nullptr) {
        NETMGR_LOG_E("context == nullptr");
        return;
...
napi_value NapiEthernet::SetIfaceConfig(napi_env env, napi_callback_info info)
{
    ...
    NETMGR_LOG_I("SetIfaceConfig agvc = [%{public}d]", static_cast<int32_t>(argc));
    ...
    NETMGR_LOG_I("SetIfaceConfig mode = [%{public}d]", context->ipMode);
...
napi_value NapiEthernet::GetIfaceConfig(napi_env env, napi_callback_info info)
{
    ...
    NAPI_CALL(env, napi_get_cb_info(env, info, &argc, argv, nullptr, nullptr));
    NETMGR_LOG_I("GetIfaceConfig agvc = [%{public}d]", static_cast<int32_t>(argc));
    EthernetAsyncContext* context = std::make_unique<EthernetAsyncContext>().release();
    // Parse Js argv
    NAPI_CALL(env, napi_get_value_string_utf8(env, argv[ARGV_INDEX_0],
        context->ethernetName, ETHERNET_NAME_MAX_BYTE, &(context->ethernetNameRealBytes)));
...
{
    NETMGR_LOG_I("IsActivate");
    size_t argc = ARGV_NUM_2;
    napi_value argv[] = {nullptr, nullptr} ;
    NAPI_CALL(env, napi_get_cb_info(env, info, &argc, argv, nullptr, nullptr));
    NETMGR_LOG_I("IsActivate agvc = [%{public}d]", static_cast<int32_t>(argc));
    EthernetAsyncContext* context = std::make_unique<EthernetAsyncContext>().release();
    // Parse Js argv
    NAPI_CALL(env, napi_get_value_string_utf8(env, argv[ARGV_INDEX_0],
        context->ethernetName, ETHERNET_NAME_MAX_BYTE, &(context->ethernetNameRealBytes)));
    NETMGR_LOG_I("IsActivate [%{public}s]", context->ethernetName);
    napi_value result = nullptr;
    if (argc == ARGV_NUM_1) {
        if (context->callbackRef == nullptr) {
            NAPI_CALL(env, napi_create_promise(env, &context->deferred, &result));
        ...
        NETMGR_LOG_E("IsActivate  exception");
    }
    // creat async work
    napi_value resource = nullptr;
    napi_value resourceName = nullptr;
...
napi_value NapiEthernet::GetActivateInterfaces(napi_env env, napi_callback_info info)
{
    ...
    NAPI_CALL(env, napi_get_cb_info(env, info, &argc, argv, nullptr, nullptr));
    NETMGR_LOG_I("GetActivateInterfaces agvc = [%{public}d]", static_cast<int32_t>(argc));
    EthernetAsyncContext* context = std::make_unique<EthernetAsyncContext>().release();
    napi_value result = nullptr;
    if (argc == ARGV_NUM_0) {
        if (context->callbackRef == nullptr) {
        ...
        NETMGR_LOG_E("GetActivateInterfaces  exception");
    }
    // creat async work
    napi_value resource = nullptr;
    napi_value resourceName = nullptr;
