...
int32_t NetConnService::UnregisterNetSupplierAsync(uint32_t supplierId, bool ignoreUid, int32_t callingUid)
{
    ...
    std::shared_lock<ffrt::shared_mutex> defaultNetSupplierUniqueLock(defaultNetSupplierMutex_);
...
void NetConnService::RequestAllNetworkExceptDefault()
{
    ...
    for (const auto &netSupplier : netSuppliers_) {
        std::shared_lock<ffrt::shared_mutex> defaultNetSupplierLock1(defaultNetSupplierMutex_);
        ...
        defaultNetSupplierLock1.unlock();
...
void NetConnService::HandleDetectionResult(uint32_t supplierId, NetDetectionStatus netState)
{
    ...
    std::shared_lock<ffrt::shared_mutex> defaultNetSupplierLock(defaultNetSupplierMutex_);
    bool isNeedRemoveDelayNetwork = defaultNetSupplier_ &&
        defaultNetSupplier_->GetNetSupplierType() != BEARER_CELLULAR;
    if (isNeedRemoveDelayNetwork) {
        RemoveDelayNetwork();
    }
    ...
    std::shared_lock<ffrt::shared_mutex> defaultNetSupplierLock1(defaultNetSupplierMutex_);
    bool isNeedRequestAllNetworkExceptDefault = defaultNetSupplier_ &&
        defaultNetSupplier_->GetSupplierId() == supplierId;
    if (!ifValid && isNeedRequestAllNetworkExceptDefault) {
...
int32_t NetConnService::UpdateSupplierScoreAsync(uint32_t supplierId, uint32_t detectionStatus)
{
    ...
    bool isNeedRequestAllNetworkExceptDefault = defaultNetSupplier_ &&
        defaultNetSupplier_->GetSupplierId() == supplierId;
    defaultNetSupplierLock.lock();
    if (isNeedRequestAllNetworkExceptDefault) {
        RequestAllNetworkExceptDefault();
    }
    return NETMANAGER_SUCCESS;
}
