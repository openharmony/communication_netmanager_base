...
int32_t NetsysNativeServiceProxy::AddIfName(const std::string &ifName)
{
    ...
    MessageParcel reply;
    MessageOption option;
    if (ERR_NONE != Remote()->SendRequest(INetsysService::NETSYS_ADD_IF_NAME, data, reply, option)) {
        NETNATIVE_LOGE("proxy SendRequest failed");
        return ERR_FLATTEN_OBJECT;
    }
    ...
    if (ret != ERR_NONE) {
        NETNATIVE_LOGE("fail to AddIfName ret= %{public}d", ret);
        return ret;
    }
    return ERR_NONE;
}
...
int32_t NetsysNativeServiceProxy::RemoveIfName(const std::string &ifName)
{
    ...
    MessageParcel reply;
    MessageOption option;
    if (ERR_NONE != Remote()->SendRequest(INetsysService::NETSYS_REMOVED_IF_NAME, data, reply, option)) {
        NETNATIVE_LOGE("proxy SendRequest failed");
        return ERR_FLATTEN_OBJECT;
    }
    ...
    if (ret != ERR_NONE) {
        NETNATIVE_LOGE("fail to RemoveIfName ret= %{public}d", ret);
        return ret;
    }
    return ERR_NONE;
}
