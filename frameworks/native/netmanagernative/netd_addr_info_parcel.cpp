...
using namespace  std;
NetdAddrInfoParcel::NetdAddrInfoParcel(const struct addrinfo* addr, const uint16_t netId,
...
bool NetdAddrInfoParcel::Marshalling(Parcel &parcel) const
{
...
sptr<NetdAddrInfoParcel>  NetdAddrInfoParcel::Unmarshalling(Parcel &parcel)
{
    Parcel *pParcel = &parcel;
    MessageParcel  *parcelMsg = static_cast<MessageParcel*>(pParcel);
    auto ptr = (make_unique<NetdAddrInfoParcel>()).release();
    if (ptr == nullptr) {
        return nullptr;
    }
